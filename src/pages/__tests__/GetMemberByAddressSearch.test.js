...
describe("GetMemberByAddressSearch Component", () => {
  ...
  test("displays list of members when multiple members are found", async () => {
    const members = [
      ...
      },
      {
        ...
        memberAddress: "123 Maple Street, Toronto",
        memberEmailAddress: "janesmith@email.com",
  ...
  test("displays error message when error is passed", async () => {
    const errorMessage = "No members found for this address";
    ...
    expect(
      screen.getByText("No members found for this address")
    ).toBeInTheDocument();
  });
});
